...
bool Ds18b20_Init_Simple(void){
	...
	OneWire.ROM_NO[1] = 0xc5;
	OneWire.ROM_NO[2] = 0x4b;
	OneWire.ROM_NO[3] = 0x94;
	OneWire.ROM_NO[4] = 0x97;
	OneWire.ROM_NO[5] = 0x03;
	OneWire.ROM_NO[6] = 0x03;
	OneWire.ROM_NO[7] = 0xf9;
...
bool	Ds18b20_Init(void)
{
	...
	do
	{
		...
		while(HAL_GetTick() < 3000)
			Ds18b20Delay(100);
		OneWireDevices = OneWire_First(&OneWire);  // find address
...
void CheckConverting(){
	m_busy = 1;
	m_isConverting = !DS18B20_AllDone(&OneWire); //finish 1 , not finish 0
	m_busy = 0;
}
